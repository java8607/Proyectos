	...
	private void setAvailability(Property typeEmployee, int cantidadEmpleados, boolean state) {
		...
		for (int i = 0; i < cantidadEmpleados; i++) {
			...
			if (typeEmployee.equals(Property.DIRECTOR) && EmployeeFactory.directors.get(i).isAvaible() != state) {
				...
				int calls = EmployeeFactory.directors.get(i).getCalls();
				EmployeeFactory.directors.get(i).setCalls(calls++);
				...
				indexEmployee = i;
				int calls = EmployeeFactory.supervisors.get(i).getCalls();
				EmployeeFactory.supervisors.get(i).setCalls(calls++);
				break;
				...
				indexEmployee = i;
				int calls = EmployeeFactory.operators.get(i).getCalls();
				EmployeeFactory.operators.get(i).setCalls(++calls);
				break;
			}
		}
	}
